...
%
% The value is (Stopped Rows) rather than the rows alone. Reaching a bound
...
% Stopped is the bound's own word from the limit vocabulary, or false, because
% the bounds have different remedies and one flag sent a caller to the wrong
% one. The Python seat carries the same fact as `Trace.stopped`.
metta_node_command(trace, [Src0, Space0, Max0], [value, Wire]) :-
    metta_node_text(Src0, Src),
    metta_node_space(Space0, Space),
    metta_node_number_arg(Max0, Max),
    metta_trace_source(Src, Space, Max, Events, Stopped),
    maplist(metta_node_trace_row, Events, Rows),
    metta_node_expr_wire(Rows, RowsWire),
    format(atom(StoppedText), "~w", [Stopped]),
    metta_node_expr_wire([[s, StoppedText], RowsWire], Wire).
